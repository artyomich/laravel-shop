    ...
    <section class="col-xs-12 content-tires" itemscope itemtype="http://schema.org/Product">
        ...
        <form class="content product-card-partner" action="/cart/add/{{ $product->id }}/">
                ...
                <div class="col-xs-9">
                    ...
                    <div class="partner-cert">
                        @if (strtolower($product->properties->brand) === 'nortec')
                            <a href="#" class="thumbnail videoLink" data-toggle="modal" data-target="#nortecAdModal">
                                <span>Рекламный ролик Nortec</span>
                            </a>
                        @endif
                        <a href="#" class="thumbnail videoLink" data-toggle="modal" data-target="#trustedVendor">
            ...
            <ul class="day-cost product-inline">
                <li class="ul-theader">
                    <div class="row-tir-days">Доставка</div>
                    <div class="tir-balance">Наличие</div>
                    <div class="tir-cost">Цена</div>
                    <div class="tir-col">Количество</div>
                ...
                $product->balance && $product->is_visible ? $balance[] = ['day' => 0, 'balance' => $product->balance->balance, 'cost' => $product->balance->cost, 'transportCost' => 0, 'vendor' => null] : '';

                $current_balances = \models\ProductsBalances::where('products_balances.product_id', $product->id)
                ->where('products_balances.city_id', null)
                ...
                ->get()
                ->toArray();
                foreach ($current_balances as $item) {
                if ($item['cdek_id']) {
                $cdek = \models\Products::getPriceCdek($item, $city);
                if(isset($cdek->result)){
                $cd=$cdek->result;
                $balance[] = ['day' => $cd->deliveryPeriodMax, 'balance' => $item['balance'], 'cost' => $item['cost']+$item['cost']*\models\Markup::where('name', 'Общая наценка')->first()->value/100, 'transportCost' => $cd->price, 'vendor' => $item['vendor_id']];
                }
                }
                }
                ?>
...
<!-- NORTEC AD MODAL -->
<div class="modal fade" id="trustedVendor" tabindex="-1">
    <div class="modal-dialog" style="width: 200px">
        <form class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span
                            class="sr-only">Close</span></button>
                <h4 class="modal-title">Надежный поставщик</h4>
            </div>
            <div class="modal-body">
                <a href="/files/images/opinions/resized/00ffeff15e98abda06f9377cb5a768ed1aa1285a_1024x768.jpg" data-lightbox="image-1" class="thumbnail">
                    <img src="/files/images/opinions/resized/00ffeff15e98abda06f9377cb5a768ed1aa1285a_145x145c.jpg" crop="1">
                </a>
            </div>
        </form>
    </div>
...
<div class="modal fade" id="cicModal" tabindex="-1">
    <div class="modal-dialog">
        <form class="modal-content" action="/cart/add/{{ $product->id }}/">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span
                            class="sr-only">Close</span></button>
