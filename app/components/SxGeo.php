...
class SxGeo
{
    ...
    public function __construct($db_file = 'SxGeo.dat', $type = SXGEO_FILE)
    {
        ...
        if ($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time']
            * $info['id_len'] == 0
        ) {
    ...
    public function get_num($ip)
    {
        ...
        if ($blocks['max'] - $blocks['min'] > $this->range) {
            ...
            $part = $this->search_idx(
                $ipn, floor($blocks['min'] / $this->range), floor($blocks['max'] / $this->range) - 1
            );
    ...
    public function about()
    {
        ...
        $types = array('n/a', 'SxGeo Country', 'SxGeo City RU', 'SxGeo City EN', 'SxGeo City', 'SxGeo City Max RU',
